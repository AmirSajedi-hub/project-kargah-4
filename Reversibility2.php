...
<form method="post" action="Reversibility3.php">
    <fieldset class='get_number'>
        <label for='number'>مرتبه ماتریس مربعی:</label>
        <?php
        echo "<input type='number' value='$number' name='number' id='number' readonly>";
        ?>
    </fieldset>
    <fieldset class="get_Equations">
        <legend style="direction: rtl; text-align: right; padding-left: 7px; padding-right: 7px;">ماتریس را وارد کنید.</legend>
        <?php
        for ($i = 0 ; $i < $number ; $i++)
        {
            for ($j = 0 ; $j < $number ; $j++)
